...
class DoomEnv(Env):
    ...
    def step(self, action: int) -> t.Tuple[Frame, int, bool, t.Dict]:
        action_l = self.possible_actions[action]
        action_l.append(1)
        reward = self.game.make_action(action_l, self.frame_skip)
    ...
    def reward(self,action):
        ...
        if self.game.get_state():
            ...
            if wall/(320*240) >=0.9:
                reward -=0.03
            if np.average(depth[110:130,150:170])<5:
                reward -=0.03
            ammo = self.game.get_game_variable(vizdoom.GameVariable.AMMO2)
            reward += max((ammo - self.ammo)*0.1,0)
            self.ammo = ammo
        self.step_count += 1
        return reward
        '''
